...
public class SouthMessagesServiceIntegrationTest {
    ...
    public void cleanUp () throws ParseException {
        session.execute(truncate(KEYSPACE, SOUTH_MESSAGES_BY_INTERVAL_TABLE));
        session.execute(truncate(KEYSPACE, SOUTH_MESSAGES_BY_SUBJECT_INTERVAL_TABLE));
        session.execute(truncate(KEYSPACE, SOUTH_MESSAGES_BY_SUBJECT_USER_INTERVAL_TABLE));
    ...
    public void given_a_repository_when_queried_by_specific_interval_then_verify_returned_messages_are_in_that_interval() throws ParseException {
        ...
        List<Row> rows = session.execute(select()
                .all()
                .from(KEYSPACE, SOUTH_MESSAGES_BY_INTERVAL_TABLE)
                .allowFiltering()
                .where(gte(OCCUR_TIME_FIELD, from))
                .and(lte(OCCUR_TIME_FIELD, to))).all();

        Page<SouthMessageByInterval> response = southMessagesService.getMessagesByInterval(from, to, null);
        assertThat(response.content, hasSize(rows.size()));
        for (SouthMessageByInterval message : response.content) {
    ...
    public void given_a_repository_with_more_rows_than_fetch_size_configured_when_queried_by_then_verify_returned_messages_are_paged() throws ParseException {
        ...
        String page = null;
        int pagesQueried = 0;
        List<SouthMessageByInterval> pagedRows = new ArrayList<>();

        Mapper<SouthMessageByInterval> mapper = mappingManager.mapper(SouthMessageByInterval.class);
        List<SouthMessageByInterval> fetchedRows = mapper.map(session.execute(select()
                .all()
                .from(KEYSPACE, SOUTH_MESSAGES_BY_INTERVAL_TABLE)
                .allowFiltering()
                .where(gte(OCCUR_TIME_FIELD, from))
                .and(lte(OCCUR_TIME_FIELD, to)))).all();
        do {
            Page<SouthMessageByInterval> response = southMessagesService.getMessagesByInterval(from, to, page, fetchSize);
    ...
    public void given_a_repository_when_queried_by_specific_interval_and_subject_then_verify_returned_messages_are_in_that_interval() throws ParseException {
        ...
        Page<SouthMessageBySubjectInterval> response = southMessagesService.getMessagesBySubjectInterval(subject, from, to, null);
        assertThat(response.content, hasSize(rows.size()));
        for (SouthMessageBySubjectInterval message : response.content) {
    ...
    public void given_a_repository_when_queried_by_specific_interval_user_and_subject_then_verify_returned_messages_are_in_that_interval() throws ParseException {
        ...
        Page<SouthMessageBySubjectUserInterval> response = southMessagesService.getMessagesBySubjectUserInterval(subject, user, from, to, null);
        assertThat(response.content, hasSize(rows.size()));
        for (SouthMessageBySubjectUserInterval message : response.content) {
            assertThat(message.getSubject(), is(subject));
    ...
    public void given_an_auditMessage_when_saved_then_its_persisted_into_all_related_tables() {
        ...
        Mapper<SouthMessageByInterval> southMessageByIntervalMapper =
                mappingManager.mapper(SouthMessageByInterval.class);
        List<SouthMessageByInterval> southMessagesByInterval = southMessageByIntervalMapper.map(
                session.execute(select().all()
                        .from(KEYSPACE, SOUTH_MESSAGES_BY_INTERVAL_TABLE).allowFiltering()
                        .where(eq(OCCUR_TIME_FIELD, occurTime)))).all();

        Mapper<SouthMessageBySubjectInterval> southMessageBySubjectIntervalMapper =
                mappingManager.mapper(SouthMessageBySubjectInterval.class);
        List<SouthMessageBySubjectInterval> southMessagesByUserInterval = southMessageBySubjectIntervalMapper.map(
                session.execute(select().all()
                        .from(KEYSPACE, SOUTH_MESSAGES_BY_SUBJECT_INTERVAL_TABLE).allowFiltering()
                        .where(eq(OCCUR_TIME_FIELD, occurTime)))).all();

        Mapper<SouthMessageBySubjectUserInterval> southMessageBySubjectUserIntervalMapper =
                mappingManager.mapper(SouthMessageBySubjectUserInterval.class);
        List<SouthMessageBySubjectUserInterval> southMessagesByUserSubjectInterval =
                southMessageBySubjectUserIntervalMapper.map(
                        session.execute(select().all()
                                .from(KEYSPACE, SOUTH_MESSAGES_BY_SUBJECT_USER_INTERVAL_TABLE).allowFiltering()
                                .where(eq(OCCUR_TIME_FIELD, occurTime)))).all();

        Payload payload = payloadService.getMessagePayload(southMessagesByInterval.get(0).getPayloadId());

        assertThat(southMessagesByInterval, hasSize(1));
        assertThat(southMessagesByUserInterval, hasSize(1));
        assertThat(southMessagesByUserSubjectInterval, hasSize(1));
        verify_payload_row_exists(session, southMessagesByInterval.get(0).getPayloadId());
        assertThat(southMessagesByInterval.get(0).getPayloadId(), allOf(
                equalTo(southMessagesByUserInterval.get(0).getPayloadId()),
                equalTo(southMessagesByUserSubjectInterval.get(0).getPayloadId())));
    ...
    public void given_a_repository_when_queried_with_wrong_paging_state_then_exception_thrown() throws ParseException {
        ...
        southMessagesService.getMessagesByInterval(new Date(), new Date(), "wrongPagingState");
    }
    ...
    public void given_a_repository_when_queried_by_a_too_big_interval_then_exception_thrown() throws ParseException {
        ...
        Date from = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("01/01/2014 0:00:00");
        Date to = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("08/01/2014 0:00:00");

        southMessagesService.getMessagesByInterval(from, to, null);
    }
    ...
    public void given_a_repository_when_queried_by_an_user_and_a_too_big_interval_then_exception_thrown() throws ParseException {
        ...
        expectedException.expectMessage("specified time range is too big, be more specific");
        given_a_repository_with_a_collection_of_persisted_messages(30);
        Date from = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("02/01/2014 0:00:00");
        Date to = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("02/02/2014 0:00:00");

        southMessagesService.getMessagesBySubjectInterval("", from, to, null);
    }
    ...
    public void given_a_repository_when_queried_by_an_user_subject_and_a_too_big_interval_then_exception_thrown() throws ParseException {
        ...
        given_a_repository_with_a_collection_of_persisted_messages(30);
        Date from = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("02/01/2014 0:00:00");
        Date to = DateFormat.getDateTimeInstance(SHORT, SHORT).parse("02/03/2014 0:00:00");

        southMessagesService.getMessagesBySubjectUserInterval("", "", from, to, null);
    }
    ...
    private void when_saved(AuditMessage auditMessage) {
        southMessagesService.save(auditMessage);
    }
