...
public class MonthBucketUnitTest {
    ...
    public void given_several_ranges_of_dates_when_get_intervals_invoked_on_bucket_then_it_returns_expected_interval_between_them () throws ParseException {
        assertThat(when_get_intervals_invoked_on_bucket(bucket, format.parse("01/01/1970 0:00:00.000"), format.parse("31/01/1970 23:59:59.999")),
                contains(1L));

        assertThat(when_get_intervals_invoked_on_bucket(bucket, format.parse("31/01/1970 23:59:59.999"), format.parse("01/02/1970 0:00:00.000")),
                contains(1L, 2L));

        assertThat(when_get_intervals_invoked_on_bucket(bucket, format.parse("31/01/1970 23:59:59.999"), format.parse("01/01/1971 0:00:00.000")),
                contains(1L, 2L, 3L, 4L, 5L, 6L, 7L, 8L, 9L, 10L, 11L, 12L, 13L));

        assertThat(when_get_intervals_invoked_on_bucket(bucket,  format.parse("25/07/2016 0:00:00.000"), format.parse("23/08/2016 22:51:00.000")),
                contains(559L, 560L));
    }
    ...
    private Map<Long, Date> given_a_collection_of_dates () throws ParseException {
        ...
        map.put(1L, format.parse("31/01/1970 23:59:59.999"));
        map.put(2L, format.parse("01/02/1970 0:00:00.000"));
        map.put(2L, format.parse("28/02/1970 23:59:59.999"));
        map.put(12L, format.parse("31/12/1970 23:59:59.999"));
        map.put(554L, format.parse("29/02/2016 23:59:59.999"));
        map.put(560L, format.parse("31/08/2016 23:59:59.999"));
        return map;
    }
